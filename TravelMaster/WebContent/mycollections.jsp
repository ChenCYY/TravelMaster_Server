...
<body class="clearfix with-menu with-shortcuts">
	...
	<c:if test="${empty requestScope.lstHotAttractions }">
		<script>
			window.location = "ShowCollectionsServlet";
		</script>
	</c:if>
            ...
            <table class="table responsive-table" id="sorting-advanced">
				<thead>
					<tr>
						...
						<th width="15%" align="center" class="align-center hide-on-mobile" scope="col">收藏时间</th>
					  ...
					  <th width="300" align="center" class="align-right" scope="col">相关操作</th>
					</tr>
				</thead>
				...
				<%int count; %>
				<%count=0; %>
					<c:forEach items="${ requestScope.lstShowCollections }" var="showcol">
					<tr>
						
						<th scope="row" class="checkbox-cell"><input type="checkbox" name="checked[]" id="check-1" value="1"></th>
						<td onclick="javascript:showInfo('${showcol.colid}');">${ showcol.aname }</td>
						<td onclick="javascript:showInfo('${showcol.colid}');">${ showcol.coldate }</td>
						<td onclick="javascript:showInfo('${showcol.colid}');">${ showcol.aprovince }</td>
					  	<td onclick="javascript:showInfo('${showcol.colid}');">${ showcol.acity }</td>
                        <td class="low-padding align-center">
                        	<button type="button" class="button compact icon-gear" onClick="return openConfirm(${ showcol.colid})">
			            		取消收藏
			            	</button>
			            	
			            	<a  href="addtravellists.jsp?aid=${ showcol.aid }&&aname=${ showcol.aname }">
			            		<button type="button" class="button compact icon-gear">
			            			加入旅程
			            		</button>
			            	</a>
			            	
			            	<a href="AttractionServlet?aid=${showcol.aid}">
			            		<button type="button" class="button compact icon-gear">
			            			查看详情
			            		</button>
			            	</a>
	...
	<ul id="shortcuts" role="complementary" class="children-tooltip tooltip-right">
		...
		<li class="current"><a href="mycollections.jsp" class="shortcut-messages" title="我的收藏">我的收藏</a></li>
	...
	<script>
		...
		{
		$.modal.confirm('确定取消收藏吗? ', function()
		{
			location="RemoveCollectionServlet?colid="+id;
		}, function()
		{
			$.modal.alert('呵呵  -_-||');
		});
